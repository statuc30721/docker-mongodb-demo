...
# Mongo database container deployment.
# Create container for mongo database in detached mode.
# Set local port on your host system running docker to 27017 and
# bind to mongo db's port which is also 27017.
# Add container name for example demo-mongodb.
# Identify the network to be used by mongodb for connections as mongo-network.
# Add environment variables for the "root" username and password.
# Identify docker image to use which in this case is named mongo.
# For data storage you can create a local folder on the host system
# that is running docker e.g. /home/<username>/datadir. 

# Please note for Windows and MacOS reference memory mapped files issue to
# avoid data corruption. A posted alternative is to use a local named volume
# reference "docker volumes".

# In this example its just a demonstration so just a typical data directory will be utilized.
# To add a basic folder just use the "-v" option for volume and a path to a folder that 
# you have permission to write to on your localhost filesystem e.g. "-v /datadir:/data/db"
